...
- ALWAYS "finish" when iteration_count >= {settings.MAX_ITERATIONS} to prevent infinite loops.
...
**Detailed Findings:**
[Comprehensive answer organised by theme]

**Confidence Assessment:**
[Overall reliability based on the fact-check results]

**Limitations & Caveats:**
[What the research could not fully address]
...
async def supervisor_decision_node(state: ResearchState, llm: ChatGroq) -> dict[str, Any]:
    """
    ...
    if iteration >= settings.MAX_ITERATIONS:
        logger.warning(
            "[Supervisor] Max iterations (%d) reached — forcing finish.", settings.MAX_ITERATIONS
        )
        return {
            "next_action": "finish",
            "iteration_count": iteration + 1,
            "messages": [
                f"[Supervisor] Max iterations ({settings.MAX_ITERATIONS}) reached. Forcing finish."
            ],
        }
...
async def supervisor_reflect_node(state: ResearchState, llm: ChatGroq) -> dict[str, Any]:
    """
...
def route_after_decision(state: ResearchState) -> str:
    """Map next_action to the corresponding graph node name."""
...
def route_after_reflection(state: ResearchState) -> str:
    """Route back to search if NEEDS_IMPROVEMENT and budget remains, else end."""
    action = state.get("next_action", "finish")
...
def _build_state_summary(state: ResearchState) -> str:
    ...
    def _status(key: str) -> str:
        ...
        return f"✅ present ({char_count} chars)"
